...
export default function AdminIndustries() {
    ...
    const [newIndustry, setNewIndustry] = useState({ name: '', is_active: true });
    const [editIndustry, setEditIndustry] = useState({ id: null, name: '', is_active: true });

    const fetchIndustries = async (page = 1, search = '') => {
        try {
            setLoading(true);
            const params = { page, search };
            const response = await axios.get('/api/react-admin/industries', { params });

            if (response.data) {
                console.log(response , " response ");
                const industryData = response.data.data.map(ind => ({
                    id: ind.id,
                    name: ind.name,
                    status: ind.is_active ? 'Active' : 'Inactive'
                }));

                setIndustries(industryData);
                setCurrentPage(response.data.current_page || 1);
                setTotalPages(response.data.last_page || 1);
                setTotalIndustries(response.data.total || 0);
            }
    ...
    useEffect(() => {
        fetchIndustries(currentPage, searchTerm);
    }, [currentPage, searchTerm]);

    const handleSearch = (e) => {
        e.preventDefault();
        setCurrentPage(1);
        fetchIndustries(1, searchTerm);
    };
    ...
    const handleConfirmDelete = async () => {
        ...
        try {
            setLoading(true);
            const response = await axios.delete(`/api/react-admin/industries/${currentIndustry.id}`);
            fetchIndustries(currentPage, searchTerm);
            setIsDeleteModalOpen(false);
            setCurrentIndustry(null);
            alert(response.data.message || 'Industry deleted successfully');
        } catch (err) {
            console.error('Error deleting industry:', err);
            alert(err.response?.data?.error || 'Failed to delete industry. Please try again.');
        } finally {
            setLoading(false);
        }
    };
    ...
    const handleAddIndustry = async () => {
        try {
            ...
            const response = await axios.post('/api/react-admin/industries', newIndustry);
            setNewIndustry({ name: '', is_active: true });
            setIsAddModalOpen(false);
            fetchIndustries(currentPage, searchTerm);
            alert(response.data.message || 'Industry created successfully');
        } catch (err) {
            console.error('Error creating industry:', err);
            const errorMsg = err.response?.data?.error || err.response?.data?.errors?.name?.[0] || 'Failed to create industry. Please try again.';
            alert(errorMsg);
        } finally {
            setLoading(false);
        }
    ...
    const handleInputChange = (e) => {
        ...
        setNewIndustry({ ...newIndustry, [name]: type === 'checkbox' ? checked : value });
    };

    const handleEditInputChange = (e) => {
        const { name, value, type, checked } = e.target;
        setEditIndustry({ ...editIndustry, [name]: type === 'checkbox' ? checked : value });
    };
    ...
    const handleEditClick = (industry) => {
        setEditIndustry({
            id: industry.id,
            name: industry.name,
            is_active: industry.status === 'Active'
        });
    ...
    const handleUpdateIndustry = async () => {
        try {
            ...
            const response = await axios.put(`/api/react-admin/industries/${editIndustry.id}`, editIndustry);
            setEditIndustry({ id: null, name: '', is_active: true });
            setIsEditModalOpen(false);
            fetchIndustries(currentPage, searchTerm);
            alert(response.data.message || 'Industry updated successfully');
        } catch (err) {
            console.error('Error updating industry:', err);
            const errorMsg = err.response?.data?.error || err.response?.data?.errors?.name?.[0] || 'Failed to update industry. Please try again.';
            alert(errorMsg);
        } finally {
            setLoading(false);
        }
        ...
        <AdminSidebarLayout>
            <div className="p-6">
                <header className="flex justify-between items-center mb-6">
                    <div><h1 className="text-3xl font-bold text-gray-900">Industries</h1></div>
                    <button
                                    ...
                                    <div className="p-6">
                                        <div className="flex justify-between items-start">
                                            ...
                                            <span className={`px-2 py-1 text-xs font-semibold rounded-full
                                                ${industry.status === 'Active' ? 'bg-green-100 text-green-800' : 'bg-red-100 text-red-800'}`}>
                                                {industry.status}
                                            </span>
                                        </div>
                                        <div className="mt-4 py-2"></div>
                                        <div className="mt-6 flex justify-end space-x-3">
                                            <button
                                                onClick={(e) => {
                                                    e.preventDefault();
                                                    handleEditClick(industry);
                                                }}
                ...
                <Modal isOpen={isDeleteModalOpen} onClose={() => setIsDeleteModalOpen(false)} title="Delete Industry">
                    <div className="p-1">
                        ...
                        <p className="text-center text-gray-600 mb-6">
                            Are you sure you want to delete the industry <span className="font-semibold">{currentIndustry?.name}</span>?
                ...
                {/* Add Industry Modal */}
                <Modal isOpen={isAddModalOpen} onClose={() => setIsAddModalOpen(false)} title="Add Industry">
                    <div className="p-1">
                        <div className="mb-6">
                ...
                {/* Edit Industry Modal */}
                <Modal isOpen={isEditModalOpen} onClose={() => setIsEditModalOpen(false)} title="Edit Industry">
                    <div className="p-1">
                        <div className="mb-6">
                        ...
                        <div className="mb-6">
                            <div className="flex items-center">
                                <input
                                    ...
                                    id="edit_is_active"
                                    name="is_active"
                                    checked={editIndustry.is_active}
                                    onChange={handleEditInputChange}
